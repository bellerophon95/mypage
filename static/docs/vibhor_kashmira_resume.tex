...
\vspace{-4pt}
\noindent Backend and platform engineer with 7+ years building event-driven systems, data pipelines, and AI infrastructure. Built high-throughput Kafka pipelines processing 10K+ msgs/sec at enterprise scale. Currently building AI agent infrastructure, context management, and evaluation systems.
...
\begin{itemize}
  \item Architected authenticated RPyC bridge on AWS EKS for streaming S3 datasets into isolated JupyterHub environments with 100\% reliability for AI evaluation workloads.
  \item Built MCP server (FastAPI) exposing tools for AI agents to load code samples, read/write to the database, and query database metadata for autonomous task execution.
  \item Designed context management layer (Supabase) decoupling heavy datasets from LLM prompts, reducing token usage by 60\% across multi-agent workflows.
  \item Deployed on AWS EKS with Docker/ECR, Prometheus, Grafana, and OpenTelemetry. Integrated FinOps practices, cutting infrastructure costs by 10\%.
\end{itemize}

\role{Software Engineer}{Jun 2017 -- Jun 2023}{Sprinklr Inc.}{Gurugram, India}
\begin{itemize}
  \item Led migration of legacy Engagement feed APIs to GraphQL, reducing response time by 15\% and network payload by 25\% at enterprise scale across Engagement and Display modules.
  ...
  \item Achieved 10\% responsiveness improvement on key Engagement paths via gRPC inter-service communication, caching optimizations, and reactive processing with Spring WebFlux. Established continuous profiling to prevent degradation.
...
\begin{itemize}
  \item Built retrieval pipeline combining Qdrant hybrid search (dense + sparse) with BGE-Reranker-v2 cross-encoder, feeding a LangGraph multi-agent loop with Self-RAG validation. Integrated Langfuse and RAGAS for production evaluation. Real-time SSE streaming of agent state and trace metrics to Next.js frontend.
\end{itemize}
